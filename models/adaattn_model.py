...
class AdaAttNModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # device = torch.device('cuda:0' if opt.gpu_ids and opt.gpu_ids[0] >= 0 else 'cpu')
        # enc_1 = nn.DataParallel(nn.Sequential(*enc_layers[:4]).to(device), opt.gpu_ids if opt.gpu_ids else [])
        ...
        device = torch.device('cuda:0' if opt.gpu_ids and opt.gpu_ids[0] >= 0 else 'cpu')
        gpu_ids_safe = opt.gpu_ids if (hasattr(opt, "gpu_ids") and len(opt.gpu_ids) > 0) else []

        enc_1 = nn.DataParallel(nn.Sequential(*enc_layers[:4]).to(device), gpu_ids_safe)
        enc_2 = nn.DataParallel(nn.Sequential(*enc_layers[4:11]).to(device), gpu_ids_safe)
        enc_3 = nn.DataParallel(nn.Sequential(*enc_layers[11:18]).to(device), gpu_ids_safe)
        enc_4 = nn.DataParallel(nn.Sequential(*enc_layers[18:31]).to(device), gpu_ids_safe)
        enc_5 = nn.DataParallel(nn.Sequential(*enc_layers[31:44]).to(device), gpu_ids_safe)
        ...
        if opt.skip_connection_3:
            ...
            self.net_adaattn_3.module.load_state_dict(torch.load("/home/admin/AdaAttN/checkpoints/AdaAttN/latest_net_adaattn_3.pth"))
        ...
        # self.net_decoder.module.load_state_dict(torch.load("/home/admin/AdaAttN/checkpoints/AdaAttN_AdaAttN_same/5_net_decoder.pth"), strict = False)
        # new replacing code starts
        import os

        decoder_weights = os.path.join("checkpoints", "latest_net_decoder.pth")
        if os.path.exists(decoder_weights):
            self.net_decoder.load_state_dict(torch.load(decoder_weights, map_location=torch.device("cpu")), strict=False)
            print(f"✅ Loaded decoder weights from: {decoder_weights}")
        else:
            print(f"⚠️ Decoder weights not found at {decoder_weights}, continuing without loading pre-trained decoder.")

        # new replacing code ends
        ...
        # new replacing code starts
        transformer_weights = os.path.join("checkpoints", "latest_net_transformer.pth")
        if os.path.exists(transformer_weights):
            self.net_transformer.load_state_dict(torch.load(transformer_weights, map_location=torch.device("cpu")), strict=False)
            print(f"✅ Loaded transformer weights from: {transformer_weights}")
        else:
            print(f"⚠️ Transformer weights not found at {transformer_weights}, continuing without loading pre-trained transformer.")

        # new replacing code starts
        # verif_weights = torch.load('/home/admin/AdaAttN/models/model_resnet18_100.pt')
        # new replacing code starts
        verif_weights_path = os.path.join("models", "model_resnet18_100.pt")
        if os.path.exists(verif_weights_path):
            verif_weights = torch.load(verif_weights_path, map_location=torch.device("cpu"))
            print(f"✅ Loaded verification model from: {verif_weights_path}")
        else:
            print(f"⚠️ Verification model not found at {verif_weights_path}, continuing without it.")
        ...
        if len(opt.gpu_ids) > 0:
            ...
            self.verif_photo = Verifier().to(torch.device("cpu"))
            print("✅ Running verifier on CPU.")
        ...
        self.seed = 6666
        if self.isTrain:
            self.loss_names = ['content', 'global', 'local', 'identity']
    ...
    def set_input(self, input_dict):
    # 🧠 Support both old and new dataset key formats
        if 'c' in input_dict:
            self.c = input_dict['c'].to(self.device)
            self.s = input_dict['s'].to(self.device)
            self.t = input_dict['t'].to(self.device)
        else:
            # FDWST local dataset version
            self.c = input_dict['blur'].to(self.device)
            self.s = input_dict['sharp'].to(self.device)
            self.t = input_dict['bin'].to(self.device)
    ...
    def forward(self):
        ...
        # self.bin_feats = self.encode_with_intermediate(self.bin) #####
        self.bin_feats = self.encode_with_intermediate(self.t)

        if self.opt.skip_connection_3:
            c_adain_feat_3 = self.net_adaattn_3(self.c_feats[2], self.bin_feats[2], self.get_key(self.c_feats, 2, self.opt.shallow_layer),
                                                   self.get_key(self.bin_feats, 2, self.opt.shallow_layer), self.seed)
        else:
            c_adain_feat_3 = None
    ...
    def compute_identity_loss(self):
        ...
        self.loss_identity = self.criterionMSE(stylized_embedding, real_embedding) * 0.0 + loss_verif_features
    ...
    def optimize_parameters(self):
        ...
        self.compute_losses()
        loss = self.loss_content + self.loss_global + self.loss_local# + self.loss_identity
        loss.backward()
