...
class AdaAttN3Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.gamma_enc = nn.DataParallel(nn.Sequential(image_encoder       ).to(opt.gpu_ids[0]), opt.gpu_ids)
        self.gamma_lin = nn.DataParallel(nn.Sequential(gamma_linear_encoder).to(opt.gpu_ids[0]), opt.gpu_ids)
        for param in self.gamma_enc.parameters() : param.requires_grad = True
        for param in self.gamma_lin.parameters() : param.requires_grad = True
        ...
        parameters.append(self.gamma_enc.parameters())
        parameters.append(self.gamma_lin.parameters())
    ...
    def forward(self):
        ...
        T_waves = torch.stack(T_waves).to(self.device)
        self.c = T_waves.permute(0, 2, 3, 1)
    ...
    def compute_blur_loss(self):
        # print("score: ", self.blur_score.shape)
        # print("gamma: ", self.gamma_blur.shape)
    ...
    def optimize_parameters(self):
        ...
        self.compute_losses()
        loss = self.loss_content + self.loss_identity + self.loss_global#+ self.loss_local
